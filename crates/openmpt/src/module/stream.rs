...
pub trait ModuleStream {
    ...
    /// # Safety
    /// This function is called by libopenmpt and must not panic.
    unsafe extern "C" fn openmpt_read(stream: *mut c_void, dst: *mut c_void, bytes: usize)
        -> usize;
}
    ...
    T: Read + Seek,
{
    ...
    unsafe extern "C" fn openmpt_tell(stream: *mut c_void) -> i64 {
        let stream_source: &mut T = &mut *(stream as *mut T);
        match stream_source.stream_position() {
            Ok(pos) => pos as i64,
            Err(_) => -1,
        }
    }
...
mod tests {
    ...
    fn check_callbacks_seekable<T: SeekableStream>(_stream: &T) {
    ...
    fn check_callbacks_non_seekable<T: ModuleStream>(_stream: &T) {
