...
impl Module {
	...
	pub fn get_pattern_by_order(&mut self, order_num: i32) -> Option<Pattern<'_>> {
	...
	pub fn get_pattern_by_number (&mut self, pattern_num: i32) -> Option<Pattern<'_>> {
...
impl<'p, 'm> Row<'p, 'm> {
	...
	pub fn get_cell_by_channel<'r> (&'r mut self, channel_num: i32) -> Option<Cell<'r, 'p, 'm>> {
		...
		if channel_num < 0 || channel_num >= num_channels {
			...
			Some(Cell{ row: self, channel_num })
		}
	}
}
