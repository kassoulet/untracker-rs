...
impl Module {
    ...
    pub fn read_float_mono(&mut self, sample_rate: i32, mono: &mut [c_float]) -> usize {
        ...
        unsafe {
            openmpt_sys::openmpt_module_read_float_mono(
                self.inner,
                sample_rate,
                count,
                mono.as_mut_ptr(),
    ...
    pub fn read_stereo(&mut self, sample_rate: i32, left: &mut [i16], right: &mut [i16]) -> usize {
        ...
        unsafe {
            openmpt_sys::openmpt_module_read_stereo(
                self.inner,
                sample_rate,
                count,
                left.as_mut_ptr(),
    ...
    pub fn read_float_stereo(
        ...
        unsafe {
            openmpt_sys::openmpt_module_read_float_stereo(
                self.inner,
                sample_rate,
                count,
                left.as_mut_ptr(),
    ...
    /// ### Returns
    /// The number of frames actually rendered (up to half of the buffer's capacity), or 0 if the end of song has been reached.
    pub fn read_interleaved_float_stereo(
        &mut self,
        sample_rate: i32,
    ...
    pub fn read_quad(
        ...
        let count = min(
            ...
            min(rear_left.len(), rear_right.len()),
        );

        unsafe {
            openmpt_sys::openmpt_module_read_quad(
                self.inner,
                sample_rate,
                count,
                left.as_mut_ptr(),
                right.as_mut_ptr(),
                rear_left.as_mut_ptr(),
                rear_right.as_mut_ptr(),
            )
        }
    }
    ...
    /// ### Returns
    /// The number of frames actually rendered, or 0 if the end of song has been reached.
    pub fn read_float_quad(
        &mut self,
        sample_rate: i32,
        left: &mut [c_float],
        right: &mut [c_float],
        rear_left: &mut [c_float],
        rear_right: &mut [c_float],
    ) -> usize {
        let count = min(
            min(left.len(), right.len()),
            min(rear_left.len(), rear_right.len()),
        ...
        unsafe {
            openmpt_sys::openmpt_module_read_float_quad(
                self.inner,
                sample_rate,
                count,
                left.as_mut_ptr(),
    ...
    pub fn read_interleaved_quad(
        ...
        unsafe {
            openmpt_sys::openmpt_module_read_interleaved_quad(
                self.inner,
                sample_rate,
                count,
                interleaved_quad.as_mut_ptr(),
            )
        }
    }
    ...
    /// ### Returns
    /// The number of frames actually rendered (up to one fourth of the buffer's capacity), or 0 if the end of song has been reached.
    pub fn read_interleaved_float_quad(
        &mut self,
        sample_rate: i32,
        interleaved_quad: &mut [c_float],
    ) -> usize {
        let count = interleaved_quad.len() >> 2; // Buffer needs to be of at least size count*4

        unsafe {
            openmpt_sys::openmpt_module_read_interleaved_float_quad(
                self.inner,
                sample_rate,
                count,
                interleaved_quad.as_mut_ptr(),
    ...
    pub fn set_repeat_count(&mut self, repeat_count: i32) -> bool {
        let return_value =
            unsafe { openmpt_sys::openmpt_module_set_repeat_count(self.inner, repeat_count) };
    ...
    fn set_render_param(&mut self, param: c_int, value: i32) -> bool {
        let return_value =
            unsafe { openmpt_sys::openmpt_module_set_render_param(self.inner, param, value) };
    ...
    pub fn get_repeat_count(&mut self) -> i32 {
        unsafe { openmpt_sys::openmpt_module_get_repeat_count(self.inner) }
    }
    ...
    fn get_render_param(&mut self, param: c_int) -> Option<i32> {
        ...
        let return_value =
            unsafe { openmpt_sys::openmpt_module_get_render_param(self.inner, param, out_ptr) };
