...
impl Module {
    ...
    /// The input data can be discarded after a Module has been constructed successfully.
    #[allow(clippy::result_unit_err)]
    pub fn create<T: stream::ModuleStream>(
        stream: &mut T,
        logger: Logger,
        init_ctls: &[ctls::Ctl],
    ) -> Result<Module, ()> {
    ...
    /// Creates a Module from a raw pointer (internal use only)
    ///
    /// # Safety
    /// This function is unsafe because it takes a raw pointer.
    /// The `owned` parameter specifies if the Module should destroy the pointer on drop.
    pub(crate) fn from_raw_pointer(ptr: *mut openmpt_sys::openmpt_module, owned: bool) -> Self {
        Module { inner: ptr, owned }
    }
}
...
pub fn could_open_propability<T: stream::ModuleStream>(
    stream: &mut T,
    effort: CouldOpenEffort,
    logger: Logger,
) -> f64 {
    let stream_ptr: *mut T = stream;

    // Use the callbacks from the stream implementation
    let callbacks = T::get_file_callbacks();

    unsafe {
        openmpt_sys::openmpt_could_open_propability(
            callbacks,
            stream_ptr as *mut _,
            effort.value(),
            logger.log_func(),
...
mod tests {
    ...
    fn empty_file_is_invalid() {
        let fake_file: Vec<u8> = Vec::new();
        let mut fake_file = Cursor::new(fake_file);
        let lazy_prob =
            could_open_propability(&mut fake_file, CouldOpenEffort::NoEffort, Logger::None);
        let probe_prob = could_open_propability(
            &mut fake_file,
            CouldOpenEffort::ProbeFileHeader,
            Logger::None,
        );
        let header_prob =
            could_open_propability(&mut fake_file, CouldOpenEffort::VerifyHeader, Logger::None);
        let load_partial_prob = could_open_propability(
            &mut fake_file,
            CouldOpenEffort::LoadWithoutPatternOrPluginData,
            Logger::None,
        );
        let load_complete_prob = could_open_propability(
            &mut fake_file,
